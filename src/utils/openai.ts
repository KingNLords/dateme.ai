...
// utils/openai.ts
import { ChatMood, UserPreferences } from "@/store/chatStore"; // Import UserPreferences
...
export class OpenAIService {
  private apiKey: string;
  // Ensure this is the correct OpenAI API base URL
  private baseURL = "https://api.openai.com/v1/chat/completions";
  ...
  async sendMessage(
    ...
    const payload = {
      model: "gpt-4o-mini", // Sticking to gpt-4o-mini as requested
      messages: [
...
- Help bridge the distance in long-distance relationships`;
    ...
    // NEW: Add MBTI information to the system prompt if available
    if (userPreferences) {
      fullPrompt += `\n\nUser Profile:`;
